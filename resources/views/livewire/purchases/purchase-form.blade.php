<div>
    <div class="flex flex-col-reverse sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-text-primary">{{ $purchase ? 'Edit Bill' : 'Record New Bill' }}</h1>
            <p class="text-sm text-text-secondary mt-1">Fill in the details from the vendor's invoice</p>
        </div>
        <a href="{{ route('purchases.index') }}" class="btn btn-ghost" wire:navigate>
    ...
    <form class="space-y-6">
                ...
                <div>
                    ...
                    @if($showNewContactInput)
                        <div class="flex items-center gap-2 mt-1">
                            <input wire:model="newContactName" type="text" class="form-input text-sm py-1.5 min-h-[2.25rem]" placeholder="New vendor name..." wire:keydown.enter.prevent="createContact">
                        ...
                        <tr>
                            <td class="px-4 py-3">
                                <div class="flex flex-col gap-2 whitespace-normal">
                                    <select wire:model.live="items.{{ $index }}.product_id" class="form-input py-1.5 text-sm cursor-pointer">
                            ...
                            <td class="px-4 py-3 align-top">
                                <select wire:model.live="items.{{ $index }}.tax_rate_id" class="form-input py-1.5 text-sm cursor-pointer">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-end gap-4 pt-4">
            <a href="{{ route('purchases.index') }}" class="btn btn-ghost w-full sm:w-auto" wire:navigate>Cancel</a>
            <button type="button" wire:click="save('draft')" class="btn btn-secondary w-full sm:w-auto">
                Save as Draft
            </button>
            <button type="button" wire:click="save('receive')" class="btn btn-primary w-full sm:w-auto">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
