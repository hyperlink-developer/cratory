<div>
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-text-secondary mt-1">Track incoming customer payments</p>
        </div>
        <a href="{{ route('receipts.create') }}" class="btn btn-primary" wire:navigate>
    ...
    <div class="glass-card p-4 mb-6 flex gap-4">
        <div class="relative flex-1">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" class="form-input pl-9" placeholder="Search by number or customer...">
        </div>
    </div>
            ...
            @forelse($receipts as $receipt)
                <div class="p-4 flex flex-col gap-3">
                    ...
                    <div class="flex items-center justify-end pt-2 border-t border-white/5">
                        <button wire:click="deleteReceipt('{{ $receipt->uuid }}')" wire:confirm="Delete this receipt and revert invoice balances?" class="p-1.5 text-text-muted hover:text-red-400 transition-colors" title="Delete">
                ...
                <tbody class="divide-y divide-white/5">
                    @forelse($receipts as $receipt)
                        <tr class="hover:bg-white/5 transition-colors group">
                            ...
                            <td class="px-6 py-4 text-right font-medium text-accent">₹{{ number_format($receipt->amount, 2) }}</td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                    <button wire:click="deleteReceipt('{{ $receipt->uuid }}')" wire:confirm="Delete this receipt and revert invoice balances?" class="p-1.5 rounded-lg text-text-muted hover:text-red-400 hover:bg-red-400/10 transition-colors" title="Delete">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-text-muted">No receipts found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
